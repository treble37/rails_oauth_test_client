class DataController < ApplicationController
  def index
    if session[:current_access_token]
      @me = JSON.parse(HTTParty.get "#{ENV['OAUTH_PROVIDER_URL']}/api/v1/users/me.json?access_token=#{session[:current_access_token]}")
    else
  ...
  def create_session
    ...
    response = JSON.parse HTTParty.post("#{ENV['OAUTH_PROVIDER_URL']}/oauth/token", req_params)
